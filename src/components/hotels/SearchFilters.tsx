          ...
          {starOptions.map(star => (
            <label key={star} className="flex items-center cursor-pointer">
              ...
              <div className="ml-3 flex items-center">
                ...
                <span className="ml-2 text-sm text-gray-900">{star} Star</span>
              </div>
            </label>
          ))}
        </div>
        ...
        <div className="space-y-3">
          {ratingOptions.map(option => (
            <label key={option.value} className="flex items-center justify-between cursor-pointer">
              ...
              <span className="text-sm text-gray-900">{option.count}</span>
            </label>
          ))}
        </div>
      </div>
          ...
          {facilityOptions.slice(0, 6).map(facility => (
            <label key={facility} className="flex items-center cursor-pointer">
              <input
                ...
                className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500"
              />
              <span className="ml-3 text-sm text-gray-900">{facility}</span>
            </label>
          ))}
        </div>
      </div>
            ...
            <label key={style} className="flex items-center justify-between cursor-pointer">
              <div className="flex items-center">
                <input
                  ...
                  className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500"
                />
                <span className="ml-3 text-sm text-gray-900">{style}</span>
              </div>
            ...
            <label key={neighborhood.name} className="flex items-center justify-between cursor-pointer">
              <div className="flex items-center">
                <input
                  ...
                  className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500"
                />
                <span className="ml-3 text-sm text-gray-900">{neighborhood.name}</span>
              </div>
              <span className="text-sm text-gray-900">{neighborhood.count}</span>
            </label>
          ))}
        </div>
      </div>
