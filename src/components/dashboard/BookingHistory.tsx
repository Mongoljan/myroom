  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <h2 className="text-base font-bold text-gray-900">{t('dashboard.bookingHistory', 'Booking History')}</h2>
      ...
      <div className="space-y-2">
        {isLoading ? (
          <div className="text-center py-10">
            ...
            <p className="text-sm text-gray-500">{t('dashboard.loadingBookings', 'Loading bookings...')}</p>
          </div>
        ) : bookings.length === 0 ? (
          <div className="text-center py-10">
            ...
            <p className="text-sm text-gray-500">{t('dashboard.noBookings', 'No bookings found')}</p>
          </div>
        ) : (
          bookings.map((booking) => (
            <div
              key={booking.id}
              className="flex items-center justify-between p-4 rounded-lg border border-gray-100 bg-gray-50 hover:bg-white hover:border-gray-200 hover:shadow-sm transition-all duration-200"
            >
              {/* Left */}
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <h3 className="text-sm font-semibold text-gray-900 truncate">{booking.hotel_name}</h3>
                </div>
                <p className="text-xs text-gray-500 mb-2">{booking.room_type}</p>
              ...
              <div className="flex flex-col items-end gap-2 ml-4">
                ...
                <p className="text-sm font-bold text-gray-900">₮{booking.total_price.toLocaleString()}</p>
      ...
      {bookings.length > 0 && (
        <div className="mt-4 pt-4 border-t border-gray-100 text-center">
          <button className="text-sm font-medium text-gray-600 hover:text-gray-900 transition-colors inline-flex items-center gap-1">
